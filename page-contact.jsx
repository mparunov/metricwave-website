...
function ContactForm({ t, mode }) {
  const SVCS = ["Analytics & BI", "Web", "Process Mining", "Automation", "Strategy", "Data Engineering"];
  const [picked,   setPicked]   = cUseState(["Analytics & BI"]);
  const [status,   setStatus]   = cUseState("idle"); // idle | sending | sent | error
  const [vals,     setVals]     = cUseState({ name: "", email: "", company: "", message: "" });

  const toggle = (s) => setPicked(p => p.includes(s) ? p.filter(x => x !== s) : [...p, s]);
  const change = (id) => (e) => setVals(v => ({ ...v, [id]: e.target.value }));

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!vals.name || !vals.email || !vals.message) return;
    setStatus("sending");
    const payload = { ...vals, "form-name": "contact", services: picked.join(", ") };
    try {
      await fetch("/", {
        method: "POST",
      ...
      // Fallback: open mailto if fetch fails (e.g. file:// local dev)
      const sub  = encodeURIComponent("New enquiry from " + (vals.name || "website"));
      const body = encodeURIComponent(
        `Name: ${vals.name}\nEmail: ${vals.email}\nCompany: ${vals.company}\nServices: ${picked.join(", ")}\n\n${vals.message}`
      );
      window.open(`mailto:mikheil@metricwave.net?subject=${sub}&body=${body}`, "_blank");
      setStatus("sent");
    }
  };
  ...
  const labelStyle = {
    ...
    fontSize: 10, color: t.text2, letterSpacing: "0.15em",
  ...
  if (status === "sent") {
    return (
      <div style={{
        background: "transparent", border: `1px solid ${t.teal}`, borderRadius: 16,
        padding: 48, textAlign: "center",
      }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>✓</div>
        <h3 style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: 26, color: t.teal, margin: "0 0 12px" }}>
          Message sent!
        </h3>
        <p style={{ color: t.text2, fontSize: 16, lineHeight: 1.6 }}>
          Thanks for reaching out. We'll get back to you within 1 working day at <strong>{vals.email}</strong>.
        </p>
      </div>
    );
  }
  ...
  return (
    <form
      ...
      style={{ background: "transparent", border: `1px solid ${t.border}`, borderRadius: 16, padding: 36 }}
    >
      ...
      {[
        ...
        <div key={f.id} style={{ marginBottom: 18 }}>
          ...
          <input
            type={f.type} name={f.id} placeholder={f.ph}
            value={vals[f.id]} onChange={change(f.id)}
      ...
      <div style={{ marginBottom: 24 }}>
        <label style={labelStyle}>What's the problem to solve? *</label>
        <textarea
          name="message" rows={5} required
          placeholder="A few sentences about what you're trying to figure out…"
          value={vals.message} onChange={change("message")}
          style={{ ...inputStyle, lineHeight: 1.5, resize: "vertical" }}
        />
      </div>

      {status === "error" && (
        <p style={{ color: t.warn, fontSize: 13, marginBottom: 12 }}>
          Something went wrong — please try again or email us directly at mikheil@metricwave.net
        </p>
      )}
      ...
      <button
        ...
        style={{
          ...
          border: "none",
          background: status === "sending" ? t.tealDk : t.teal,
          color: t.bg, width: "100%",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {CLIENTS.map((c, i) => (
              <div key={c.city} style={{
                ...
                background: "transparent", border: `1px solid ${c.featured ? t.teal : t.border}`,
                display: "flex", justifyContent: "space-between", alignItems: "center",
              }}>
                <div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 32 }}>
            {[
              ...
              <div key={row.lbl} style={{ padding: "16px 20px", borderRadius: 10, background: "transparent", border: `1px solid ${t.border}` }}>
